...
{
    imu660ra_get_gyro(); // 采集陀螺仪数据
    imuComputer();
    Circle.now_yaw=eulerAngle.yaw-cha_yaw;

        // 添加Yaw角度累积记录
        // if (yaw_record_enable)
        // {
...
{
    encoder_process();
    // steering_control(Err);
   
    //  Speed_ClosedLoop_Control(target_left_speed, target_right_speed);
    // Speed_Control_Test();
    if (system_running)
    {
        if (Break.have)
        ...
        {
             Speed_ClosedLoop_Control();
          //  Speed_Control_Test();
        }
        Motor_Ctrl((int)output_left, (int)output_right); // 运行模式下使用PID输出
    }
    // else if (ImageFlag.stop)
    // {
    //     Motor_Ctrl(100, 100); // 停车模式下电机停止
    // }
    else
    {
        Motor_Ctrl(0, 0); // 菜单模式下电机停止
    }
