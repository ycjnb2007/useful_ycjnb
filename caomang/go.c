...
void go(void)
{
    ...
    if(disappear_flag == 0 && get_start_point()) //如果找到起点
    {
        ...
        Annulus_flag = Go_Annulus(); //环岛判断

        if(!Annulus_flag)  //十字处理
        {
            Stop_line = Deal_crossroads();
        }

        if(Stop_line == 0 && !Annulus_flag)  //常规巡线
        {
        ...
        stopline_flag += Get_stopline(); //停车线数目统计
    }
    
    disappear_flag = Deal_disappear();
    if(elements_index >= 9 || elements_Lock[elements_index] == Empty_)
    {
      elements_index = 0;
    }
    directionControl();
