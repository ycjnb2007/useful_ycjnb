...
Point line_points[MT9V03X_W + CUT_HEIGHT]; 
uint16 num_points;
...
{
    const float peak_center = 0.30f;  // 权重峰值位置（0.6=中间偏上）
    const float peak_width = 0.3f;    // 高权重区域宽度
    ...
    for (uint16 i = 0; i < num_points; i++) 
    {
        ...
        float distance = fabsf(normalized_pos - peak_center);
        
        if (distance <= peak_width / 2) 
        {
            out_weights[reversed_idx] = max_weight;  // 核心区域最高权重
        } else 
        {
            // 边缘区域平滑衰减
            float falloff = (distance - peak_width/2) / (0.5f - peak_width/2);
...
{
    static float dynamic_weights[MAX_POINTS];  // 静态内存分配
...
//{
//  uint8 count = 0;
//  for(uint8 i = col_start_line; i < col_start_line+ many_count;i++)
//  {
//    for(uint8 j = row_last_line; j < row_start_line + 1; j--)
//    {
//      if(Trip_point(image[j][i],image[j - 1][i]))
//      {
//        left_col_count[count].x = i;
//        left_col_count[count].y = j;
//        count++;
//      }
//    }
//  }
...
uint8 Element_col_left(void) 
{
    ...
    for (uint8 i = col_start_line; i < col_start_line + many_count && total_count < point_count; i++) 
    {
...
{
  uint8 count = 0;
  for(uint8 i = col_start_line; i < col_start_line + many_count1;i++)
  {
    for(uint8 j = row_last_line; j > row_start_line + 1; j--)
    {
      if(Trip_point(image[j][i],image[j - 1][i]))
...
uint8 Element_col_right(void) 
{
    ...
    for (uint8 i = col_last_line; i > col_last_line - many_count && total_count < point_count; i--) 
    {
        uint8 col_jump_count = 0;
        Point temp_points[point_count];
...
{
  uint8 count = 0;
  for(uint8 i = col_last_line; i > col_last_line - many_count1; i--)
  {
    for(uint8 j = row_last_line; j > row_start_line + 1; j--)
    {
      if(Trip_point(image[j][i],image[j - 1][i]))
...
//{
//    uint8 count = 0;
//    for(uint8 i = row_start_line; i < row_start_line + many_count; i++)
//    {
//      for(uint8 j = col_start_line; j < col_last_line- 1;j++)
//      {
//         if(Trip_point(image[i][j], image[i][j + 1]))
//         {
//            up_row_count[count].x = j;
//            up_row_count[count].y = i;
//            count++;
//         }
//      }
//    }
...
uint8 Element_row_up(void) 
{
    ...
    for (uint8 i = row_start_line; i < row_start_line + many_count && total_count < point_count; i++) 
    {
...
{
    uint8 count = 0;
    for(uint8 i = row_start_line; i < row_start_line + many_count1; i++)
    {
      for(uint8 j = col_start_line; j < col_last_line- 1;j++)
      {
         if(Trip_point(image[i][j], image[i][j + 1]))
...
//{
//    uint8 count = 0;
//    for(uint8 i = row_last_line; i < row_last_line - many_count; i--)
//    {
//      for(uint8 j = col_start_line; j < col_last_line- 1;j++)
//      {
//         if(Trip_point(image[i][j], image[i][j + 1]))
...
uint8 Element_row_down(void) 
{
    ...
    for (uint8 i = row_last_line; i > row_last_line - many_count && total_count < point_count; i--) 
    {
        uint8 row_jump_count = 0;
        Point temp_points[point_count]; 
...
{
    uint8 count = 0;
    for(uint8 i = row_last_line; i > row_last_line - many_count1; i--)
    {
      for(uint8 j = col_start_line; j < col_last_line- 1;j++)
      {
         if(Trip_point(image[i][j], image[i][j + 1]))
