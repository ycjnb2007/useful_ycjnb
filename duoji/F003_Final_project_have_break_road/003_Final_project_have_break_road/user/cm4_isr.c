...
void pit0_ch2_isr()                     // 定时器通道 2 周期中断服务函数      
{
    pit_isr_flag_clear(PIT_CH2);
        ...
        if(start_servo % 2 == 1)    //通过按键开启
        {
            switch(Trackstate)          
                {
                  case LINE_TRACKING:       //正常寻迹状态
                        differential_k = differential_k_normal;
                        servo(Mid_point);
                        peak_center_use = PEAK_CENTER_ROUNDABOUT;
                        
                        break;
                    
                  case ROUNDABOUT:          //六边形环岛状态
                        differential_k = differential_k_roundabout;
                        servo_control_roundabout(Mid_point);
                        peak_center_use = PEAK_CENTER;
                         break; 
                         
                   case LOST_LINE:          //断路丢线状态
                          
                        servo_pwm_control(Break_road_read_select[0]);
...
void pit0_ch10_isr()                    // 定时器通道 10 周期中断服务函数      
{
    ...
    switch(car_state_current)
    {
    case STATE_NORMAL:
            exchange_show_page_control(); 
            break;
    case STATE_WRITE:
            Key_control_break_road();
            break;
